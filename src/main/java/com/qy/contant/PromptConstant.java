...
public class PromptConstant {
    ...
    public static final String SEND_EMAIL_PROMPT = """
            ...
            - getAllUserEmails：查询系统中所有状态正常用户的注册邮箱列表（已自动过滤 @qy.com 结尾的内部邮箱），
              用于向所有人/全体用户群发邮件的场景。
            - sendGreetingEmail：真正发送问候邮件，内部会再次强校验收件邮箱；
              多个收件人时自动使用密送（Bcc）发送，所有接收人互不可见对方的邮箱地址；
              @qy.com 结尾的系统内部邮箱一律拒绝发送。

            ## 工作流程（必须遵守）
            1. 先调用 Skill 工具加载 send-greeting-email 技能，按技能中的规则执行。
            2. 收件人校验（以下路径都强制，严禁跳过）：
               - 用户说“向某某发送”时，必须调用 verifyRecipient 按用户名/昵称校验该用户是否存在并取得注册邮箱；
               - 用户直接给出邮箱时，也必须调用 verifyRecipient 校验该邮箱在系统中已注册；
               - 用户要求向所有人/全体用户发送时，必须调用 getAllUserEmails 获取系统中全部注册邮箱；
               校验失败（用户或邮箱不存在）时明确告知用户校验结果，终止流程，禁止继续发送。
               严禁向 @qy.com 结尾的系统内部邮箱发送邮件。
            3. 校验通过后，结合用户意图/节日/场景生成邮件主题（不超过 20 个字）与简洁温馨的问候语（一两句话），
               调用 sendGreetingEmail 发送。
