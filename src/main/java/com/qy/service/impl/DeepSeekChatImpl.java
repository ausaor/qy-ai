...
/**
 * DeepSeek 对话服务实现
 * 基于 Spring AI 2.0 的 ChatClient 与大模型进行流式对话
 */
@Slf4j
...
public class DeepSeekChatImpl implements IChatService {
    ...
    /** DeepSeek 默认聊天选项（模型/温度/最大token数） */
    ...
    /**
     * 构造器注入：通过 @Qualifier 明确指定 DeepSeek 专属的 Bean，
     * 避免与千问的同类型 Bean 产生歧义
     */
    public DeepSeekChatImpl(
            @Qualifier("deepseekChatOptions") OpenAiChatOptions chatOptions,
    ...
    public Flux<ChatResponse> streamMessage(ChatMessageRequest request) {
        log.info("DeepSeek 流式发送消息到会话: sessionId = {}, content = {}", request.getSessionId(), request.getContent());
    ...
    public Flux<String> streamChat(ChatRequest chatRequest) {
        log.info("DeepSeek 流式发送消息到会话: sessionId = {}, content = {}", chatRequest.getSessionId(), chatRequest.getContent());
